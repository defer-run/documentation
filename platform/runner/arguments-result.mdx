...
The arguments passed to a background function **must be serializable**.
They will be serialized using `JSON.stringify` upon enqueuing.
...
They will be serialized using `JSON.stringify` and stored after the execution is done.
