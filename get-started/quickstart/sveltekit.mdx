...
In a SvelteKit app, we recommend creating a `defer/` folder as follows:
...
```tsx src/routes/hello-world/+server.ts
...
Now, with your API deployment containing your Defer Token and your project's builds ready on Defer, open your browser and test the SvelteKit route.

In the `helloWorld()` background function scenario, visit the `/hello-world` route.
