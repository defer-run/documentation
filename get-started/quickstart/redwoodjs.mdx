...
description: "Learn how to setup Defer and trigger your first background function."
...
to organize both your background functions and client as follows:
...
```tsx api/src/jobs/defer/helloWorld.ts
...
<Tabs>
  ...
  <Tab title='Move existing slow code or service to a background function'>
    ...
    ```tsx api/src/jobs/defer/syncContacts.ts
    // the `defer()` helper will be used to define a background function
    import { defer } from "src/jobs/clients/defer";
    // the `defer()` helper will be used to define a background function
...
```tsx api/src/functions/helloWorld/helloWorld.ts
...
If you are wondering how calling a background function behaves locally, please refer to this guide: [Development & Testing](/get-started/development-testing).
...
let's ensure that your Defer token and Build Settings are correctly configured.
