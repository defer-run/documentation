...
### Configure Next.js (`>= 13.5.1` only)

<Note>
  If your **Next.js** version is less than `13.5.1`, you can skip this step
</Note>

Starting version `13.5.1`, the `serverMinification` experimental feature is
emabled by default. This does not play well with the Defer Builder.

To use Defer, please edit your `next.config.js` file and disable it:
