...
[`defer()`](/references/defer-client/defer) to transform it into a Defer
...
We'll leverage Defer's [`delay()`](/references/defer-client/delay) feature to
match this criteria:
...
Each call to `sendMonthlyUsage()` from the NextJS App will trigger a background
execution on **Defer**.
