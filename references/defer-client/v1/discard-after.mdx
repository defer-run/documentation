...
The `discardAfter(deferFn, interval)` function takes a deferred function and
applies a custom `discard_after` attribute. By default, the `discard_after` is
not set.
...
`discardAfter()` will be dropped in `2.0`.
