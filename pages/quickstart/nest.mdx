...
Learn how to setup Defer with Nest and trigger your first background function.
...
In a Nest app, we recommend creating an `defer/` folder as follows:
...
Using Nest Services from background functions requires some minor plumbing work since background functions behave as what Nest calls "standalone apps".

<br />

For example, your Nest application relies on a standard `PrismaService`:

```ts filename="src/prisma.service.ts" copy
...
The `getPrismaInstance()` initialize a Nest Application context that will initiate the Services with your application configuration.
...
```tsx filename="src/app.controller.ts" {3, 11} copy
...
Now that your Nest app contains a background function and an API route that calls it, let's ensure that your Defer token is properly configured.
...
Now, with your API deployment containing your Defer Token and your project's builds ready on the Defer Platform, open your browser and test the Nest Controller route.
