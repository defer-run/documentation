...
In a Express/Koa/hapi app, we recommend creating an `src/defer/` folder as follows:
...
<Tabs items={['Move existing existing slow code to a background function', 'Write a new feature, ex: syncContacts()']}>
  ...
  <Tab>
    ...
    ```tsx filename="defer/syncContacts.ts" copy
    // the `defer()` helper will be used to define a background function
    import { defer } from "@defer/client"
    // the `defer()` helper will be used to define a background function
...
Whenever you choose to use the `helloWorld()` function or create your own, the following example showcasing how to call `helloWorld()` from a Express, Koa or Hapi API route demonstrates how easily you can trigger a background function execution.

<br />

<Tabs items={['Express', 'Koa', 'hapi']}>

<Tab>
  ```tsx filename="src/api.ts" {4, 10-11} copy
...
<Tab>
```tsx filename="src/api.ts" {3, 8-9} copy
import Koa from 'koa';
// we import our `helloWorld()` background function
import helloWorld from "../../defer/helloWorld";
...
<Tab>
  ```tsx filename="src/api.ts" {4, 17-18} copy
...
<div className={'image'}>
<div><Image src="/getting-started/build-success.png" alt="Your first successful build on the Defer Console" width={400} height={450} /></div>
</div>
...
<div className={'image'}>
<div><Image src="/getting-started/env-var-vercel.png" alt="Setup your Defer Token on Vercel" width={800} height={900} /></div>
</div>
...
<div className={'image'}>
<div><Image src="/getting-started/env-var-heroku.png" alt="Setup your Defer Token on Vercel" width={800} height={900} /></div>
</div>
...
<div className={'image'}>
<div><Image src="/getting-started/env-var-render.png" alt="Setup your Defer Token on Vercel" width={800} height={900} /></div>
</div>
...
Now, with your API deployment containing your Defer Token and your project's builds ready on the Defer Platform, open your browser and test your `/helloWorld` API route.
